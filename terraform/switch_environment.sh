...
if [[ -f environments/${STAGE}/backend.config ]]; then
    ...
    echo "Running: terraform init -backend-config=environments/${STAGE}/backend.config -reconfigure"
    terraform init -backend-config=environments/${STAGE}/backend.config -reconfigure
else
